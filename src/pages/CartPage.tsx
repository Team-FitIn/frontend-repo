...
const INITIAL_CART: CartItem[] = [
  { id: 1, title: "Minimalist Knitwear Sweater", price: 89000, image: "/Main-page_img/2.1.png", category: "Knit", selectedSize: "M", quantity: 1, sizes: ["S", "M", "L", "XL"] },
  { id: 2, title: "Raw Denim Casual Jacket", price: 129000, image: "/Main-page_img/3.1.png", category: "Denim", selectedSize: "L", quantity: 1, sizes: ["S", "M", "L", "XL"] },
];
              ...
              <AnimatePresence>
                {cartItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      {/* 썸네일 */}
                      <div className={`w-20 h-20 rounded-xl flex-shrink-0 flex items-center justify-center overflow-hidden ${style?.bg || "bg-gray-50"}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            ...
                            className="w-6 h-6 rounded-lg bg-gray-100 hover:bg-red-50 text-gray-400 hover:text-red-400 flex items-center justify-center transition-colors cursor-pointer flex-shrink-0"
                          >
                            <X size={12} />
                          </button>
                        </div>
                        ...
                        <div className="flex items-center gap-3 mt-2.5">
                          ...
                          <div className="relative">
                            <button
                              onClick={() => setOpenSize(openSize === item.id ? null : item.id)}
                              className="flex items-center gap-1 px-2.5 py-1 border border-gray-200 rounded-lg text-[11px] font-semibold text-gray-600 hover:border-[#3D1E5F]/40 transition-colors cursor-pointer"
                            >
                              {item.selectedSize}
                            ...
                            <AnimatePresence>
                              {openSize === item.id && (
                                <motion.div
                                  ...
                                  animate={{ opacity: 1, y: 0 }}
                                  exit={{ opacity: 0, y: -4 }}
                                  className="absolute top-8 left-0 bg-white border border-gray-100 rounded-xl shadow-lg z-10 overflow-hidden"
                                >
                                  {item.sizes.map(size => (
                                    <button
                                      key={size}
                                      onClick={() => updateSize(item.id, size)}
                                      className={`block w-full px-4 py-1.5 text-[11px] font-semibold text-left hover:bg-violet-50 hover:text-[#3D1E5F] transition-colors cursor-pointer ${
                                        item.selectedSize === size ? "bg-violet-50 text-[#3D1E5F]" : "text-gray-600"
                                      }`}
                                    >
                                      {size}
                                    </button>
                          ...
                          {/* 금액 */}
                          <p className="text-[14px] font-black text-[#3D1E5F] ml-auto">
            ...
            <div className="mt-6 p-4 bg-white rounded-2xl border border-gray-100">
              <div className="flex flex-col gap-2 mb-4">
                ...
                <div className="flex justify-between">
                  <span className="text-[13px] font-semibold text-gray-700">총 결제 금액</span>
                  <span className="text-[17px] font-black text-[#3D1E5F]">₩{total.toLocaleString("ko-KR")}</span>
                </div>
              </div>
