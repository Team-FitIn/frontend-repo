...
const MOCK_GARMENTS: Garment[] = [
  { id: 1, name: "클래식 화이트 셔츠", brand: "무신사 스탠다드", category: "상의", sizes: ["S", "M", "L", "XL"] },
  { id: 2, name: "슬림핏 블랙 팬츠", brand: "아더에러", category: "하의", sizes: ["S", "M", "L"] },
  { id: 3, name: "데님 재킷", brand: "리바이스", category: "아우터", sizes: ["S", "M", "L", "XL"] },
  { id: 4, name: "니트 스웨터", brand: "마르디 메크르디", category: "상의", sizes: ["S", "M", "L"] },
  { id: 5, name: "롱 울 코트", brand: "코스", category: "아우터", sizes: ["S", "M", "L"] },
  { id: 6, name: "캐주얼 티셔츠", brand: "반스", category: "상의", sizes: ["S", "M", "L", "XL"] },
  { id: 7, name: "와이드 데님", brand: "리바이스", category: "하의", sizes: ["S", "M", "L"] },
  { id: 8, name: "후드 집업", brand: "나이키", category: "아우터", sizes: ["M", "L", "XL"] },
  { id: 9, name: "린넨 셔츠", brand: "무신사 스탠다드", category: "상의", sizes: ["S", "M", "L", "XL"] },
  { id: 10, name: "슬랙스 팬츠", brand: "폴로", category: "하의", sizes: ["S", "M", "L", "XL"] },
  { id: 11, name: "오버핏 셔츠", brand: "커버낫", category: "상의", sizes: ["M", "L", "XL"] },
  { id: 12, name: "패딩 점퍼", brand: "노스페이스", category: "아우터", sizes: ["S", "M", "L", "XL"] },
];
...
export default function VirtualFittingPage() {
  ...
  const handleTryOn = async () => {
    ...
    // if (faceImage) formData.append("user_face", faceFile);
    // const res = await fetch(`/api/v1/fitting/try-on/${fitList[0].garment.id}`, {
    //   method: "POST",
    //   headers: { Authorization: `Bearer ${token}` },
    //   body: formData,
    // });
    // const blob = await res.blob();
    // setFitResult(URL.createObjectURL(blob));
    await new Promise((r) => setTimeout(r, 2000));
    setFitResult(faceImage || "/Main-page_img/1.1.png");
    setIsFitting(false);
  };
  ...
  const canFit = fitList.length > 0 && !isFitting;
  ...
  return (
    <div className="flex h-[calc(100vh-73px)] overflow-hidden bg-[#FAFAFA]">
      ...
      {/* ── 가운데: 의류 그리드 ── */}
      <main style={{ flexBasis: "40%", minWidth: 0 }} className="flex flex-col overflow-hidden border-r border-gray-100">
            ...
            {filteredGarments.map((garment, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative bg-white rounded-xl overflow-hidden cursor-pointer transition-all ${isSelected ? "ring-1 ring-[#3D1E5F] shadow-sm shadow-violet-100/60" : "border border-gray-100 hover:border-gray-200 hover:shadow-sm"}`}
                >
                  <div className={`aspect-square relative flex items-center justify-center ${style?.bg || "bg-gray-50"}`}>
                    {garment.imageUrl ? <img src={garment.imageUrl} alt={garment.name} className="w-full h-full object-cover" /> : <span className={`text-xl font-black opacity-20 ${style?.text || "text-gray-400"}`}>{garment.category[0]}</span>}
      ...
      <aside style={{ flexBasis: "45%", minWidth: 0 }} className="bg-white flex flex-col overflow-hidden">
        <div className="flex flex-col items-center gap-2 p-3 bg-gradient-to-b from-gray-50 to-white border-b border-gray-100 relative flex-shrink-0">
          {(faceImage || fitList.length > 0) && (
            <button onClick={handleReset} className="absolute top-2 right-2 w-5 h-5 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors cursor-pointer">
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2.5 py-1 text-[10px] rounded-lg transition-all cursor-pointer font-medium ${faceImage ? "bg-[#3D1E5F]/10 text-[#3D1E5F] border border-[#3D1E5F]/20" : "bg-white border border-dashed border-gray-300 text-gray-500 hover:border-[#3D1E5F] hover:text-[#3D1E5F]"}`}
          >
            <Camera size={11} />
            {faceImage ? "사진 변경" : "얼굴 사진 업로드 (선택)"}
          </button>
          <div className="w-24 h-44 rounded-[28px] bg-white border border-gray-200 overflow-hidden flex items-center justify-center shadow-sm">
            {isFitting ? (
          ...
          <motion.button
            ...
            onClick={handleTryOn}
            disabled={!canFit}
            className={`w-full py-2 rounded-xl text-[11px] font-bold transition-all flex items-center justify-center gap-1.5 ${canFit ? "bg-[#3D1E5F] text-white shadow-md shadow-violet-200 cursor-pointer" : "bg-gray-100 text-gray-300 cursor-not-allowed"}`}
          >
            <Sparkles size={11} />
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <AnimatePresence mode="wait">
            {selectedGarment ? (
              <motion.div key={selectedGarment.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex flex-col gap-3">
                <div>
                  ...
                  <p className="text-[13px] font-bold text-gray-800 leading-snug">{selectedGarment.name}</p>
                </div>
                <div>
                  <p className="text-[9px] text-gray-400 font-bold mb-2 uppercase tracking-wide">Size</p>
                  <div className="flex gap-1.5 flex-wrap">
                    {selectedGarment.sizes.map((s) => (
                      <button key={s} onClick={() => handleSizeChange(s)} className={`min-w-[30px] h-7 px-2 text-[11px] rounded-lg border font-semibold transition-all cursor-pointer ${selectedSize === s ? "border-[#3D1E5F] text-[#3D1E5F] bg-violet-50" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  <button onClick={() => toggleFitList(selectedGarment)} className={`w-full py-2.5 rounded-xl text-[12px] font-bold transition-all flex items-center justify-center gap-1.5 cursor-pointer ${isInFitList(selectedGarment.id) ? "bg-red-50 text-red-400 border border-red-200" : "bg-[#3D1E5F] text-white hover:bg-[#2D1545]"}`}>
              ...
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center gap-2 py-6">
                ...
                <p className="text-[10px] text-gray-400 text-center leading-relaxed">카드를 클릭하면<br />상세 정보가 표시됩니다</p>
              </motion.div>
            )}
          </AnimatePresence>
        </div>
